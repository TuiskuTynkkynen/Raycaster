...
void UILayer::OnUpdate(Core::Timestep deltaTime) {
    ...
    if (sceneState != RaycasterScene::State::Running) {
        ...
        if (sceneState >= RaycasterScene::State::Dead && Core::UI::Button("Restart", glm::vec2{ 0.5f, 0.25f })) {
            Core::Application::PushEvent<Restart>();
        }

        if (sceneState == RaycasterScene::State::Paused && Core::UI::Button("Continue", glm::vec2{ 0.5f, 0.25f })) {
            Core::Application::PushEvent<Resume>();
        }

        if (Core::UI::Button("Quit", glm::vec2{ 0.5f, 0.25f })) {
